 ...
 $existSql = "SELECT * FROM `books` WHERE bookname = '$bookname'";
    ...
    if($numExistRows > 0){
        // $exists = true;
          ...
          <span class="block sm:inline">Book already listed.</span>
          <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
          </span>
        </div>';
    }
else{
    $sql = "INSERT INTO `books` (`sno`, `bookname`, `author`, `ISBN`, `price`, `dt`) VALUES (NULL, '$bookname', '$author', '$isbn', '$price', current_timestamp())";
    $result = mysqli_query($conn,$sql);
    if($result){
        echo '<div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
        <strong class="font-bold">Success!</strong>
        <span class="block sm:inline">New book has been listed.</span>
        <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
        </span>
      </div>';
    }
...
<body class="bg-slate-400">
    <h1 class="text-center mt-44 text-4xl font-semibold">Insert New Book</h1>
    <br>
    <button class= "bg-blue-600 text-white p-2.5 flex mx-auto rounded mt-3" ><a href="../dashboard.php">Go back</a></button>
    <br>
<form method="post" action="insert.php" class="w-1/2 mx-auto mb-44 shadow-2xl bg-slate-600 p-12 rounded-xl">
  ...
  <div class="relative z-0 mb-6 w-full group">
      <input type="text" name="author" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="">
  ...
  <div class="relative z-0 mb-6 w-full group">
      <input type="text" name="isbn" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="">
