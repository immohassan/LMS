...
if($username == $_SESSION["username"]){
    $sql = "INSERT INTO `lendings` ( `username`, `bookname`, `return_date`, `dt`) VALUES ( '$username', '$bookname', '$date', current_timestamp())";
    $result= mysqli_query($conn,$sql);
    if($result){
        echo '<div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
            <strong class="font-bold">Success!</strong>
            <span class="block sm:inline">Book lent successfully!</span>
    ...
    <span class="block sm:inline">Please enter your valid username.</span>
    <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
    </span>
    <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
...
<h1 class="text-center mt-40 text-4xl font-semibold">Lent Books</h1>
    <br>
    <button class= "bg-blue-600 text-white p-2.5 flex mx-auto rounded mt-3" ><a href="home.php">Go back</a></button>
    <br>
<form method="post" action="addlend.php" class="w-1/2 mx-auto mb-44 shadow-2xl bg-slate-600 p-12 rounded-xl">
  ...
  <div class="relative z-0 mb-6 w-full group">
      <input type="text" name="bookname" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required="">
  ...
  <div class="relative z-0 mb-6 w-full group">
      ...
      <label for="floating_repeat_password" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Return date(yyyy-mm-dd)</label>
  </div>
